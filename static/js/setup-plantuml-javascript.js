//  The original file is "https://plantuml.github.io/plantuml.js/plantuml-wasm/plantuml.js".

const PLANTUML = (() => {
  const initialize = async (cheerpjPath, plantumlPath) => {
    await Promise.all([
      cheerpjInit({ preloadResources: _runtimeResources() }),
      _preloadPlantumlFiles(plantumlPath)
    ])

    // to make cjcall work, first we load the java package like this
    await cheerpjRunMain("com.plantuml.wasm.v1.RunInit", `${cheerpjPath}/plantuml-core.jar`, `${cheerpjPath}/plantuml`)
  }
